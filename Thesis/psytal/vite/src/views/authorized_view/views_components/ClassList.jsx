      ...
      <div className="w-full h-[auto] px-4 mx-auto rounded-b-3xl bg-white shadow-2xl max-h-[400px] overflow-y-auto">
        <table className="w-full">
          <tbody >{filteredData.map((subject, index) => (
                ...
                <td className="w-[10%] text-left p-2" onClick={() => this.handleOpenPopUp(subject)}>{subject.class_code}</td>
                <td className="w-[10%] text-left p-2" onClick={() => this.handleOpenPopUp(subject)}>{subject.course_code}</td>
                <td className="w-[25%] text-left p-2" onClick={() => this.handleOpenPopUp(subject)}>{subject.course_title}</td>
                <td className="w-[10%] text-left p-2" onClick={() => this.handleOpenPopUp(subject)}>{subject.semester}</td>
                <td className="w-[10%] text-left p-2" onClick={() => this.handleOpenPopUp(subject)}>{subject.class_year}</td>
                <td className="w-[10%] text-left p-2" onClick={() => this.handleOpenPopUp(subject)}>{subject.class_section}</td>
                <td className="w-[15%] text-left p-2" onClick={() => this.handleOpenPopUp(subject)}>{subject.instructor_name}</td>
                <td className="w-[10%] text-left p-2">
